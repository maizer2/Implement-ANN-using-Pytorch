from network.AE.vanila_AE import AutoEncoder
...
def get_opt():
    ...
    parser.add_argument("--save_root", default="train/AE/VanilaAE")
    ...
    for epoch in tqdm(range(0, opt.num_epochs + 1)):
        for imgs, _ in train_loader:
            ...
            if epoch % opt.check_point == 0:
                ...
                writer.add_scalar(f"VanilaAE/Scalar/Loss", loss.item(), epoch)
                writer.add_image(f"VanilaAE/Image/Real", real_grid, epoch)
                writer.add_image(f"VanilaAE/Image/Fake", fake_grid, epoch)
