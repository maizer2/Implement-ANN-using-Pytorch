...
def train_AE(
    ...
    save_root: str = "train/GAN/AE/checkpoint/"
):
    ...
    model = AutoEncoder(img_channels).to(device)
    ...
    writer = SummaryWriter("Tensorboard/AE/")
    ...
    for epoch in tqdm(range(0, num_epochs + 1)):
        for imgs, _ in train_loader:
            ...
            if epoch % check_point == 0:
                ...
                writer.add_image(f"AE/Image/Real/batch:{batch_size},lr:{lr}", real_grid, epoch)
                writer.add_image(f"AE/Image/Fake/batch:{batch_size},lr:{lr}", fake_grid, epoch)
                writer.add_scalar(f"AE/Loss/batch:{batch_size}, lr:{lr}", loss.item(), epoch)
