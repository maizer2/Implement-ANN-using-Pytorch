...
def train_DCGAN_mul(
    ...
    netG = DDP(netG, [gpu_id], broadcast_buffers=False)
    netD = DDP(netD, [gpu_id], broadcast_buffers=False)
    ...
    writer = SummaryWriter("Tensorboard/GAN/DCGAN_mul")
    ...
    for epoch in tqdm(range(0, num_epochs + 1)):
        ...
        for imgs, _ in train_loader:
            ...
            x = imgs.to(gpu_id)
            fake = netG(torch.randn((x.size(0), latent_space_vector, 1, 1)).to(gpu_id))
